...
(function(){
	...
	var search = function(query){ //Fixed error: By adding closing {
		...
		var queryArray = query.split(" "); //fixed error; join to split
		...
		for(var i=0, j=db.length; i<j; i++) { //declaring loop through data base
            ...
            var dbitem = db[i].toLowerCase().substring(0, dbTitleEnd); //Fixed error: camel case toLowerCase

            // loop through the user's search query words
            // save a lowercase variable of the search keyword
            for (var ii = 0, jj = queryArray.length; ii < jj; ii++) { //loop length
                var qitem = queryArray[ii].toLowerCase(); //Fixed error: camel case toLowerCase
	...
	var showMatches = function(results){ // declaring var for matches
		...
		for(var i=0, j=results.length; i<j; i++){ //loop through data base
			...
			title = results[i].substring(0, titleEnd); //fixed error :subString to substring
